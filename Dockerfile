...
COPY --from=builder /paper.jar paper.jar
...
CMD mkdir -p plugins world/datapacks && \
    ...
    mv /conf/ops.json ./ ; chown minecraft:minecraft ops.json ; \
    mv /conf/whitelist.json ./ ; chown minecraft:minecraft whitelist.json ; \
    echo eula=true > eula.txt && \
    setpriv --reuid=minecraft --regid=minecraft --init-groups --inh-caps=-all /run-server.sh ${HEAP_SIZE}
